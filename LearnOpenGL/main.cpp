...
#pragma region 渲染循环
	while (!glfwWindowShouldClose(window)) // 每次循环的开始前检查一次GLFW是否被要求退出
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			// LightDirectional
			glUniform3f(
				glGetUniformLocation(myShader->id, "lightDirectional.pos"),
				light.position.x,
				light.position.y,
				light.position.z
			); // 手动设置纹理单元
			glUniform3f(
				glGetUniformLocation(myShader->id, "lightDirectional.color"),
				light.color.x,
				light.color.y,
				light.color.z
			); // 手动设置纹理单元
			glUniform3f(
				glGetUniformLocation(myShader->id, "lightDirectional.dirToLight"),
				light.direction.x,
				light.direction.y,
				light.direction.z
			); // 手动设置纹理单元
			...
			// LightPoint
			// glUniform1f(glGetUniformLocation(myShader->id, "lightPoint.constant"), light.constant); // 手动设置纹理单元
			// glUniform1f(glGetUniformLocation(myShader->id, "lightPoint.linear"), light.linear); // 手动设置纹理单元
			// glUniform1f(glGetUniformLocation(myShader->id, "lightPoint.quadratic"), light.quadratic); // 手动设置纹理单元
